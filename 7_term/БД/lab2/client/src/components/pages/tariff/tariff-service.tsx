...
const TariffServiceConfigurationTable: React.FC<{}> = ({}) => {
    const apiName = "tariffService";
    ...
    const [rows, setRows] = useState<TariffService[]>([]);
    ...
    const fetchData = async () => {
        try {
            ...
            setRows(data.map((row) => ({...row, id: row.tariffName + row.serviceName})));
        }
        catch(e) {
            console.error(e)
        }
    ...
    return (
        <Box>
            <Typography sx={{ marginBottom: "20px"}} variant="h2">Tariff/Service</Typography>
...
export default memo(TariffServiceConfigurationTable)
