...
const TypeConfigurationTable: React.FC<{}> = ({}) => {
    const apiName = "type";
    ...
    const fetchData = async () => {
        try {
            setIsLoading(true);
            const {data} = await methods[apiName].get();
    ...
    return (
        <Box>
            <Typography sx={{ marginBottom: "20px"}} variant="h2">Type</Typography>

            <Typography sx={{ marginBottom: "20px", maxWidth: "800px"}}>
                На этой странице представлены все типы тарифов провайдера и их особенности (скидка, премиумность)
            </Typography>
...
export default memo(TypeConfigurationTable)
